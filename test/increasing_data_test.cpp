 ...
 * @brief Class for testing push to control tower.
 */
class IncreasingDataTest : public rclcpp::Node {
 public:
  /// @brief Constructor of IncreasingDataTest.
  IncreasingDataTest(rclcpp::NodeOptions options)
      : Node("nturt_push_to_control_tower_node", options),
        ...
        fast_update_data_timer_(this->create_wall_timer(
            100ms,
            std::bind(&IncreasingDataTest::fast_update_data_timer_callback,
                      this))),
        slow_update_data_timer_(this->create_wall_timer(
            1s, std::bind(&IncreasingDataTest::slow_update_data_timer_callback,
                          this))),
        send_data_timer_(this->create_wall_timer(
            10ms,
            std::bind(&IncreasingDataTest::send_data_timer_callback, this))) {
    // init can frame data
 ...
 private:
  ...
  void fast_update_data_timer_callback() {
    ...
    INCREMENT_DATA(inv_tx_.INV_Temperature_Set_3.INV_Motor_Temp_phys, 1, 0,
                   100);
    INCREMENT_DATA(inv_tx_.INV_Current_Info.INV_DC_Bus_Current_phys, 1, 0, 100);
  ...
  void slow_update_data_timer_callback() {
    ...
    INCREMENT_DATA(vcu_status->VCU_Error_Code, 1, 0, 1);

    // rear_sensor_status
    REAR_SENSOR_Status_t* rear_sensor_status =
        &rear_sensor_tx_.REAR_SENSOR_Status;
    INCREMENT_DATA(rear_sensor_status->REAR_SENSOR_Status, 1, 0, 4);
    ...
    INCREMENT_DATA(inverter_fault_codes->INV_Post_Fault_Lo, 1, 0, 1);
    INCREMENT_DATA(inverter_fault_codes->INV_Run_Fault_Lo, 1, 0, 1);
    ...
    // gps_fix
    gps_fix_.header.stamp = this->now();
    ...
    // gps_vel
    gps_vel_.header.stamp = this->now();
  ...
  /// @brief ROS2 publisher to "/fix", for receiving GPS signal.
  rclcpp::Publisher<sensor_msgs::msg::NavSatFix>::SharedPtr gps_fix_pub_;

  /// @brief ROS2 publisher to "/vel", for receiving GPS signal.
...
/* global variable -----------------------------------------------------------*/
std::shared_ptr<IncreasingDataTest> increasing_data_test_node_public;
...
int main(int argc, char** argv) {
  ...
  auto increasing_data_test_node =
      std::make_shared<IncreasingDataTest>(options);
  increasing_data_test_node_public = increasing_data_test_node;

  executor.add_node(increasing_data_test_node);
  executor.spin();
  rclcpp::shutdown();
  return 0;
}
...
uint32_t __get__tick__() {
  return static_cast<uint32_t>(
      increasing_data_test_node_public->get_clock()->now().nanoseconds() /
      1000000);
}
...
int __send_can_message__(uint32_t msgid, uint8_t ide, uint8_t* d, uint8_t len) {
  ...
  increasing_data_test_node_public->can_pub_->publish(msg);
  return 0;
}
